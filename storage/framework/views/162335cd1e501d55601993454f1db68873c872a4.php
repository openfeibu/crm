<script type="text/html" id="barDemo">
    ...
    <a class="layui-btn layui-btn-primary layui-btn-sm" lay-event="new_window"><?php echo e(trans('goods.name')); ?></a>
</script>
...
<script>
    var order_handle = {};
    var order_lang = eval(<?php echo json_encode(trans('order')); ?>);
    var order_btn_class = eval(<?php echo json_encode(config('model.order.order.btn_class')); ?>);
        ...
        order_handle = {
            ...
            pay:function (obj) {
                layer.open({
                    ...
                    ,area: ['520px', '240px'] //宽高,
                    ,title:'<?php echo e(trans('order.operation.pay')); ?>'
                    ...
                    ,yes: function(index, layero){
                        ...
                        var load = layer.load();
                        $.ajax({
                            url : "<?php echo e(guard_url('order/pay')); ?>",
                            data :  {'id':obj['id'],'_token' : "<?php echo csrf_token(); ?>",'payment_id':payment_id,'payment_sn':payment_sn},
                            type : 'POST',
            ...
            to_delivery:function (obj) {
                layer.open({
                    ...
                    ,skin: 'layui-layer-rim' //加上边框
                    ,area: ['520px', '240px'] //宽高,
                    ,title:'<?php echo e(trans('order.operation.to_delivery')); ?>'
                    ...
                    ,yes: function(index, layero){
                        ...
                        var load = layer.load();
                        $.ajax({
                            url : "<?php echo e(guard_url('order/to_delivery')); ?>",
                            data :  {'id':obj['id'],'_token' : "<?php echo csrf_token(); ?>",'tracking_number':tracking_number},
                            type : 'POST',
                            success : function (data) {
                                layer.closeAll();
                                if(data.code == 0)
            ...
            new_window: function (obj) {
                var goods_list = '<table class="layui-table">\
                        <thead>\
                        <tr>\
                        <th width="380"><?php echo e(trans('goods.name')); ?></th>\
                        <th width="120"><?php echo e(trans('goods.label.size')); ?></th>\
                        <th width="80"><?php echo e(trans('app.number')); ?></th>\
                        <th width="120"><?php echo e(trans('app.remark')); ?></th>\
                        </tr>\
                        </thead>\
                        <tbody>';
                ...
                layer.open({
                    ...
                    closeBtn: 1,
                    shadeClose: true,
                    content: "<?php echo e(guard_url('order')); ?>/"+obj['id']
                });
                */
            },
            cancel:function (obj) {
                layer.confirm('该操作无法撤回，确定取消吗？',{title:'提示'},function(index){
                    ...
                    var load = layer.load();
                    $.ajax({
                        url : "<?php echo e(guard_url('order/cancel')); ?>",
                        data :  {'id':obj['id'],'_token' : "<?php echo csrf_token(); ?>"},
                        type : 'POST',
                        success : function (data) {
                            layer.close(load);
                            if(data.code == 0)
            ...
            receive:function (obj) {
                layer.confirm('该操作无法撤回，确定已收获吗？',{title:'提示'},function(index){
                    ...
                    var load = layer.load();
                    $.ajax({
                        url : "<?php echo e(guard_url('order/receive')); ?>",
                        data :  {'id':obj['id'],'_token' : "<?php echo csrf_token(); ?>"},
                        type : 'POST',
                        success : function (data) {
                            layer.close(load);
            ...
            return_order:function (obj) {
                layer.confirm('该操作无法撤回，确定退货吗？',{title:'提示'},function(index){
                    ...
                    var load = layer.load();
                    $.ajax({
                        url : "<?php echo e(guard_url('order/return')); ?>",
                        data :  {'id':obj['id'],'_token' : "<?php echo csrf_token(); ?>"},
                        type : 'POST',
                        success : function (data) {
                            layer.close(load);
