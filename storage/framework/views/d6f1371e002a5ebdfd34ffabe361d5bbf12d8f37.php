<script>
    var freight_config = eval(<?php echo json_encode(freight_config()); ?>);
    var freight_area_code = "<?php echo $freight_area_code; ?>";
    layui.use(['element',"table",'form',"jquery"], function(){
        ...
        function handle_number() {
            ...
            //paypal取整，去小数
            paypal_fee = parseInt(((selling_price + freight) * parseFloat("<?php echo e(setting('paypal_fee')); ?>")) * 100)/100;
        ...
        function appendTbody(node) {
            ...
            if(typeof(order_id)!="undefined" && order_id.length>0)
            {
                ...
                var load = layer.load();
                $.ajax({
                    url : "<?php echo e(guard_url('order_goods')); ?>",
                    data : ajax_data,
                    type : 'POST',
        ...
        form.on('select(customer)', function(data){
            ...
            var ajax_data = {'_token':"<?php echo csrf_token(); ?>",id:customer_id};
            var load = layer.load();
            $.ajax({
                url : "<?php echo e(guard_url('get_customer')); ?>",
                data : ajax_data,
                type : 'get',
                success : function (data) {
                    layer.close(load);
